...
import (
	...
	"github.com/usesapient/cli/internal/testclient"
	"net/http"
	"strconv"
	"time"
)
...
func NewClient(cmd *cobra.Command) (*sdk.Sapient, error) {
...
func buildGlobalSecurity(cmd *cobra.Command) components.Security {
	...
	sapientAPIKeyAuth, _ := config.ResolveSecurityCredential(cmd, "sapient-api-key-auth")
	globalSecurity := components.Security{}
	if sapientAPIKeyAuth != "" {
		globalSecurity.SapientAPIKeyAuth = sapientAPIKeyAuth
	}
