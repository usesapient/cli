...
func runWhoamiCmd(cmd *cobra.Command, args []string) error {
	...
	fmt.Fprintf(out, "Environment prefix: SAPIENT_\n")
	fmt.Fprintln(out)
	fmt.Fprintln(out, "Credentials:")

	// API Key
	{
		value, source := config.ResolveSecurityCredential(cmd, "sapient-api-key-auth")
		fmt.Fprintf(out, "  --%-25s [%-7s] %s\n", "sapient-api-key-auth", source, maskSecret(value))
	}
