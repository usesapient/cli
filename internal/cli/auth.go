...
func initAuthCmd(parent *cobra.Command) error {
	authCmd := &cobra.Command{
		...
		Long: `Manage authentication credentials for sapient.
...
Subcommands:
  ...
  logout  - Clear all stored credentials`,
	...
	authCmd.AddCommand(&cobra.Command{
		...
		Long: `Interactively configure authentication credentials for sapient.
...
func runAuthLoginCmd(cmd *cobra.Command, args []string) error {
	...
	if noInteractive, _ := cmd.Flags().GetBool("no-interactive"); noInteractive {
		...
		changed := false
		if f := cmd.Flags().Lookup("sapient-api-key-auth"); f != nil && f.Changed {
			v, _ := cmd.Flags().GetString("sapient-api-key-auth")
			if config.KeyringAvailable() {
				if err := config.SetKeyringValue("sapient-api-key-auth", v); err != nil {
					cfg.Security.SapientApiKeyAuth = v // keyring failed, store in config
				}
			} else {
				cfg.Security.SapientApiKeyAuth = v // no keyring, store in config
			}
			changed = true
		}
		...
		var authSapientApiKeyAuth string
		...
		fields := []huh.Field{
			huh.NewInput().
				Title("API Key").
				Description("--sapient-api-key-auth").
				EchoMode(huh.EchoModePassword).
				Placeholder(maskSecret(cfg.Security.SapientApiKeyAuth)).
				Value(&authSapientApiKeyAuth),
		}
		...
		if authSapientApiKeyAuth != "" {
			if config.KeyringAvailable() {
				if err := config.SetKeyringValue("sapient-api-key-auth", authSapientApiKeyAuth); err != nil {
					cfg.Security.SapientApiKeyAuth = authSapientApiKeyAuth // keyring failed, store in config
				}
			} else {
				cfg.Security.SapientApiKeyAuth = authSapientApiKeyAuth // no keyring, store in config
			}
		}
...
func runAuthLogoutCmd(cmd *cobra.Command, args []string) error {
	...
	if config.KeyringAvailable() {
		_ = config.DeleteKeyringValue("sapient-api-key-auth")
	}
	cfg.Security.SapientApiKeyAuth = ""
